...
impl<R> RecordingReader<R> {
        ...
        R: Read,
    {
        self.bytes.clear();
        let value =
            read_value(self).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))?;
...
fn request(
    ...
    write_value(writer, &msg).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))?;
    writer.flush()
}
...
fn is_redraw_notification(value: &Value) -> bool {
    ...
    items.len() == 3 && items[0].as_i64() == Some(2) && items[1].as_str() == Some("redraw")
}
...
fn capture_nvim_ui_notifications() -> io::Result<Vec<CapturedMessage>> {
    ...
    let mut stdin = child
        .stdin
        .take()
        .ok_or_else(|| io::Error::new(io::ErrorKind::BrokenPipe, "missing stdin"))?;
    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| io::Error::new(io::ErrorKind::BrokenPipe, "missing stdout"))?;
    ...
    let lines = (0..80)
        .map(|i| Value::from(format!("line {i}: abcdefghijklmnopqrstuvwxyz0123456789")))
        .collect::<Vec<_>>();
    ...
    for command in [
        ...
        request_command(&mut stdin, &mut reader, &mut msgid, command, &mut captured)?;
    }

    drain_redraw_notifications(&mut stdin, &mut reader, &mut msgid, &mut captured)?;
...
fn capture_nvim_ui_scroll_notifications() -> io::Result<Vec<CapturedMessage>> {
    ...
    let _ = request_command(&mut stdin, &mut reader, &mut msgid, "quit", &mut captured);
    drop(stdin);
