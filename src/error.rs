...
impl From<Value> for Box<CallError> {
    fn from(val: Value) -> Box<CallError> {
        match val {
            Value::Array(mut arr) if arr.len() == 2 && arr[0].is_i64() && arr[1].is_str() => {
                let s = arr
                    .pop()
...
impl Error for LoopError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match *self {
            ...
            LoopError::DecodeError(ref e) => Some(e.as_ref()),
        }
    }
}
...
impl LoopError {
    ...
    pub fn is_reader_error(&self) -> bool {
        if let LoopError::DecodeError(ref err) = *self
...
impl Display for LoopError {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        match *self {
            ...
            Self::DecodeError(ref e) => write!(fmt, "Error reading message: {e}"),
