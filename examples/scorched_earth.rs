...
fn the_larger(first: Option<(u64, u64)>, second: (u64, u64)) -> Option<(u64, u64)> {
...
fn the_smaller(first: Option<(u64, u64)>, second: (u64, u64)) -> Option<(u64, u64)> {
    if first.map(|t| t <= second) == Some(true) {
        first
    } else {
        Some(second)
...
impl Handler for NeovimHandler {
    ...
    async fn handle_notify(&self, name: String, args: Vec<Value>, neovim: Neovim<TokioFile>) {
        match name.as_ref() {
            "cursor-moved-i" => {
